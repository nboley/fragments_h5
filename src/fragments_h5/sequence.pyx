...
cdef object convert_py_string_to_c_string(
    ...
    for sequence in sequences:
        assert isinstance(sequence, bytes), \
            "Expected 'bytes', but was passed a sequence of type {}".format(type(sequence))
...
def profile(seq_len, n_seq, n_test_iterations):
    ...
    import timeit
    sequence = 'A'*seq_len
    sequences = [sequence for x in range(n_seq)]
...
def RC_map(base):
    ...
    if base == <char> b'N':
        return b'N'
    raise ValueError(f"Unrecognized base '{base}'")
...
def sample_random_seqs(n_sims, seq_len):
    return ["".join(random.choice('ACGT') for j in range(seq_len)).encode()
