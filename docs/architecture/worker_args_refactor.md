...
**(Former state, `v2.12.1` — see Status above for what replaced it.)** `build_sub_fragments_h5(args)` took one positional parameter, `args`, and destructured it on a single line into 17 names. The tuple was built at exactly one pack site, inside the contig × chunk loop in `build_fragments_h5`.

**There was a third positional accessor, and it is the one that had actually broken.** In the same function, `build_fragments_h5`, in the block computing the total progress-bar count:

```python
total_bases = sum(a[4] - a[3] for a in args)  # chunk_stop - chunk_start
```

This was a raw-index read of the same tuples, in the same function, ~370 lines away from the unpack. Any design that only reconciled pack-vs-unpack would not have addressed the site with the demonstrated failure history. (Current state: this line is now `total_bases = sum(a.chunk_stop - a.chunk_start for a in args)`, in the same location, and no longer subscriptable — see §7 for the mutation that proves it.)

### 1.1 Why positional was unsafe here (EXECUTED — read of the pack site at `v2.12.1`)
...
**In scope:** the shape of the data passed from `build_fragments_h5` to `build_sub_fragments_h5`; the positional read/write sites — the pack site (the `args.append(...)` call inside `build_fragments_h5`'s contig × chunk loop, now `SubBuildArgs(...)`), the unpack (the single-line destructure at the top of `build_sub_fragments_h5`, now 17 one-to-one rebinds), the derived accessor (the `total_bases = sum(...)` line in `build_fragments_h5`) — plus the two test monkeypatch spies that read the packed tuple positionally, `tests/test_fragment_selection.py:353` and `:453` (required changes, not optional — see §4 step 6; now `args.bam_contig`, confirmed current). The call to `build_sub_fragments_h5(arg)` on the serial (non-parallel) path is a pass-through consumer (it receives one packed object and forwards it, reading nothing positionally) and needs confirming but is not itself a positional-access site.

**Out of scope, explicitly:**

- **`max_tlen` for single-end.** Declared at `fragment.py:559`, never referenced in the SE body — dead, but the shared `input_to_fragments(...)` call inside `build_sub_fragments_h5` (the one passing `max_tlen=MAX_FRAG_LENGTH`) passes it unconditionally and `single_end_bam_to_fragments` has no `**kwargs`, so removing it raises `TypeError` on every SE build. This was proven by execution previously and the removal retracted. **Hazard note only: do not touch it.**
- **The 4-element return tuple** `(output_contig, chunk_start, chunk_stop, ofname_or_None)` — the two `return` statements inside `build_sub_fragments_h5`, unpacked at both the parallel-path result handler and the serial-path call site in `build_fragments_h5`. It is a genuine second positional shape and deserves the same treatment. It is excluded here because it is only 4 elements, is unpacked immediately at both consumers with no third derived accessor, and has no defect history. Folding it in doubles the diff for the weaker half of the problem. If pursued, it must be a **separate later commit**, not a stage of this one. (Confirmed still true post-refactor: this return shape was untouched by Stage 1.)
- `se_max_fragment_length` semantics (live at `fragment.py:591` gating a hard-limit `ValueError`; the actual SE length filter is separately in `build_sub_fragments_h5`, the `if single_end and se_max_fragment_length is not None and (fragment.stop - fragment.start) > se_max_fragment_length:` line — an older note calling it dead was wrong).
- The S3 `abspath` bug; the missing `--contig-name-map` test coverage (since closed — see `tests/test_contig_name_map.py`); annotating the rest of `fragments_h5.py`; the `build-fragments-h5` PATH errors in the test env; `pytest-timeout` not being installed; any cross-repo change.

**API surface (EXECUTED — grep, against `v2.12.1`):** `build_sub_fragments_h5` appeared only at its `def`, the parallel-path pack loop, and the serial-path call site, plus monkeypatch spies in `tests/test_fragment_selection.py:353` and `:453` (which unconditionally read the packed argument positionally, `args[1]  # bam_contig` — must be updated, see §4 step 6; now updated to `args.bam_contig`, confirmed current) and a stale line-number mention in a docstring at `tests/test_cli_validation.py` (that docstring has since been fixed to cite the gate's condition/function by name instead of a line number, closing this exact hazard). It is not in `__init__.py` (`__all__ = ["FragmentsH5"]`) and not in `main.py`. **Zero external callers — the signature change breaks no public API.** The test sites above are therefore part of this change's scope, alongside `src/`. Precedent for an internal-only change of this kind (`a5fe651`/`452b214`): the change commit itself carries no version bump; `pyproject.toml` stayed at `2.12.0` through both, and a separate `Release 2.12.1` commit (`b44bd7d`) bumped it afterward. See §4.
...
- **Fork safety:** start method is an explicit `multiprocessing.get_context('fork')` in `build_fragments_h5` (the `forkserver`→`fork` switch landed in `ebfb7fc`, whose commit message reads "Added s3 support"). The parent's `pysam` handles (`with pysam.AlignmentFile(input_fname) as bam_fp:` and its FASTA counterpart) are inside `with` blocks that close before `args` and the Pool are built; the worker opens its own FASTA via `pysam.FastaFile(fasta_filename)` inside `build_sub_fragments_h5`; `_pool_worker_init` only ignores SIGINT; module globals are constants and are never mutated (one, `methyl_keys`, is a mutable list but is never written). **No hidden fork dependency — a plain-data struct does not touch this boundary.**
- **`chdir`:** the worker body runs inside `with _temporary_working_directory():`, which `os.chdir`s into a fresh temp dir. All paths must already be absolute at pack time (`tmp_dir_name` carries the comment "tmp_dir_name is absolute path from main process" at its use site). The struct changes nothing here, and **the design must not move any path resolution into the worker.**

### 3.3 Shape

```python
# IMPLEMENTED — this exact shape is now `src/fragments_h5/fragments_h5.py`'s
# SubBuildArgs, module-scope, immediately above build_sub_fragments_h5; see §7
...
Field order mirrors the old (`v2.12.1`) tuple exactly. This is deliberate: with keyword-only construction the order is semantically irrelevant, and preserving it keeps the pack-site diff a pure `(` → `SubBuildArgs(` + `name=` annotation, reviewable line-by-line against the old tuple.
...
### Stage 1 — the whole positional-access removal, one commit — **DONE**

The steps below are recorded as originally proposed (numbered, imperative) with each now annotated with its shipped location as a content anchor rather than a `v2.12.1` line number. The commit removed *all* positional access, because a partial conversion would have left exactly the split-brain state that caused the v2.10.1 defect.

1. **Done.** `SubBuildArgs` is defined at module scope in `fragments_h5.py`, immediately above `build_sub_fragments_h5`.
2. **Done.** Pack site: the `args.append(SubBuildArgs(input_fname=..., bam_contig=..., ...))` call, inside `build_fragments_h5`'s contig × chunk loop — all 17 keyword.
3. **Done.** Worker: the old one-line 17-name unpack was replaced with **17 one-to-one rebinds** at the top of `build_sub_fragments_h5`:
   ```python
   input_fname = args.input_fname
   bam_contig = args.bam_contig
   ...
   ```
   The remaining ~150 lines of the worker body were **untouched**, including the `is_fragment_file(...)` / `single_end` dispatch that selects `input_to_fragments`, the shared `input_to_fragments(...)` generator call (the one passing `max_tlen=MAX_FRAG_LENGTH`), the `_se_kwargs` conditional, and the `min_mapq` normalisation (`_min_mapq = min_mapq if min_mapq is not None else DEFAULT_MIN_MAPQ`). The diff was mechanical and locally verifiable.
4. **Done.** The derived accessor in `build_fragments_h5` now reads `total_bases = sum(a.chunk_stop - a.chunk_start for a in args)`.
5. **Done, confirmed.** Serial path: the `else` branch (guarded by `if num_processes is not None and num_processes != 1:`) calls `build_sub_fragments_h5(arg)` and unpacks its 4-element return the same way as the parallel-path result handler. No signature change was needed under this design, so this was a no-op — confirmed exercised (see §5.2), since it is the path most tests actually run.
6. **Done, required not conditional.** Updated the two monkeypatch spies in `tests/test_fragment_selection.py:353` and `:453` (`called_contigs.append(args[1])  # bam_contig` → `args.bam_contig`) — confirmed current at those line numbers. Both inspect the packed argument positionally, unconditionally. Verified against a prototype: before the fix, `test_contig_with_zero_mapped_reads_skipped` and `test_pe_contig_with_single_mapped_read_skipped` fail with `TypeError: 'SubBuildArgs' object is not subscriptable` (2 failed, 6 passed); after `args[1]` → `args.bam_contig` at both lines, the full suite matches baseline exactly.
...
**Honest coverage gaps, as of the original `v2.12.1` draft (EXECUTED — fixture and test survey):**
- There is **no stored SE fixture**; `--single-end` is only exercised via in-test pysam-built BAMs.
- **`--contig-name-map` had ZERO test coverage anywhere.** It is the flag that makes `output_contig != bam_contig`. Since `output_contig` is one of the 17 fields — and is the very field whose insertion caused the v2.10.1 defect — the refactor touched genuinely untested territory at the time. The md5 matrix could not close this. **This gap has since been closed:** `tests/test_contig_name_map.py` now covers renaming, partial maps, `--contigs`/`allowed_contigs` interaction, and the CLI TSV map-file parser — including an explicit GC-value (not-NaN) assertion on both the renamed and the coexisting-unmapped contig, which is the assertion shape that actually detects a wrong-name FASTA lookup (mutation-tested: forcing the FASTA-lookup fallback to `None` unconditionally is caught by 2 of the file's 6 tests).
...
**Baseline (EXECUTED, at the time of the original draft):** `65 passed, 0 failed, 6 errors`. All 6 errors are the module-scoped `target_h5_path` fixture calling `subprocess.run("build-fragments-h5 ...", shell=True, check=True)` and getting `/bin/sh: 1: build-fragments-h5: not found`, exit 127 — an environment PATH problem, verified by reading real stderr, not a code defect. (This count is an illustrative snapshot, not current: the suite has grown since, chiefly with `tests/test_contig_name_map.py`. Correct baseline as of Stage 1 landing, run with `build-fragments-h5` on `PATH` so the 6 errors above don't occur: `77 passed, 0 failed, 0 errors`.)
...
**Multi-argument worker via `pool.starmap` / `starmap_async`.** Rejected on measured grounds. **EXECUTED**, 5 tasks with delays `[0.6, 0.1, 0.1, 0.1, 0.1]` submitted in index order: `imap_unordered` with a single tuple arg returned `1,2,3,4,0` (lazy, unordered); `starmap` blocked the full 0.618 s and returned in submission order (eager, ordered); `starmap_async().get()` returned from submission in 0.012 s but `.get()` blocked 0.619 s — all-or-nothing, no partial delivery. The full public method list of `multiprocessing.pool.Pool` is `apply, apply_async, close, imap, imap_unordered, join, map, map_async, starmap, starmap_async, terminate` — **there is no lazy + unordered multi-argument method.** The `tqdm(total=total_bases, ...)` progress bars — one per branch, parallel and serial — consume `total_bases` incrementally, which is exactly what streaming buys. The one-argument shape is genuinely forced.

**`functools.partial`-bound invariant config + a small per-chunk struct.** Real and tempting: ~12 of the 17 fields are invariant across every task, and only 5 vary per chunk (`bam_contig`, `output_contig`, `chunk_start`, `chunk_stop`, `contig_length`). **EXECUTED:** `imap_unordered` + `functools.partial` works, with the same streaming/unordered behaviour. Rejected anyway, on three grounds: (a) it changes the worker's *signature*, which forces a corresponding change on the serial-path call site too, widening the diff in the riskiest function; (b) it creates **two** structures where one suffices, and a future field then needs a judgement call about which one it belongs in — a new way to get it wrong, in a refactor whose purpose is removing ways to get it wrong; (c) the only benefit is pickle bytes, already measured as negligible. Against the repo's stated preference for minimum complexity, one flat struct wins.

**Fold in the 4-element return tuple.** Rejected for *this* commit — §2. Doubles the diff for the weaker half of the problem. Note that the deferred merge of worker outputs into the output h5 (the "Now that all workers are done, open the output HDF5 file and merge results" comment in `build_fragments_h5`, done only after all workers finish, for HDF5 fork-safety) is unaffected either way.
...
The B+ rating's stated reason — "the design itself has not been executed" — no longer holds. An independent reviewer prototyped `SubBuildArgs` exactly as specified (module scope, `frozen=True, slots=True`, 17 keywords at the pack site, 17 rebinds at the top of `build_sub_fragments_h5`, `a.chunk_stop - a.chunk_start` as the derived accessor) and got builds succeeding at np=1/2/4 that were **md5-identical to v2.12.1 on all six §5.1 matrix cells**, and the full suite at `65 passed, 6 errors` — identical to baseline — once the two test spies (§4 step 6) were fixed. All of §5.3's post-refactor mutation rows with a concrete predicted error message (delete a `name=`, restore the old subscript-based accessor, insert a required field with no default) hit that message verbatim. §5.2 (argument-capture diff) was also run, in this pass — see above — and came out clean. **This is no longer just a prototype: Stage 1 has since shipped on this branch (see Status, top of document), and §5.1's baseline is now `77 passed, 0 failed, 0 errors`** (see §5.3 note).
...
**Weakest points:**
- **(Closed.) `--contig-name-map` had zero coverage anywhere**, so `output_contig != bam_contig` was untested — and `output_contig` is precisely the field whose insertion caused the released defect. The md5 matrix could not close this gap, and §5.2's argument-capture diff used the default `contig_name_map=None` and so did not exercise this case either. This was the single largest residual risk when this document was written. `tests/test_contig_name_map.py` now covers it directly (see §5.1), including a mutation-tested GC-value assertion on the renamed and the coexisting-unmapped contig.
...
**What a reviewer should verify (checked against the code as shipped on this branch; locate each site by grepping for the quoted name/expression, not by line number — line numbers in this file have already drifted once):**
1. That the pack site has exactly 17 `name=` keywords and that each name's source expression is the same expression that occupied that tuple position at `v2.12.1` — line-by-line against `git show v2.12.1:src/fragments_h5/fragments_h5.py` at the old pack site. This is the one check no automated test performs. **Confirmed** (§5.2's argument-capture diff came out byte-identical).
2. That the derived accessor in `build_fragments_h5` reads `a.chunk_stop - a.chunk_start` and that **no other subscript of any element of `args`** remains anywhere in `src/` **or `tests/`** — `grep` for `a\[` and `args\[` in both, not just inside `build_fragments_h5`. This is what catches the two test spies. **Confirmed**: both `tests/test_fragment_selection.py:353` and `:453` now read `args.bam_contig`.
3. That the serial branch (`build_sub_fragments_h5(arg)` under the `num_processes is not None and num_processes != 1` guard) was actually exercised, not just read. Most of the test suite runs `num_processes=1`, so a break there is loud — but confirm it ran. **Confirmed** by the passing suite.
4. That `SubBuildArgs` is at module scope, not nested inside `build_fragments_h5`. Nested definitions pickle-fail under both start methods, and the failure would only appear on the `num_processes > 1` path. **Confirmed**: it is defined at module scope, immediately above `build_sub_fragments_h5`.
5. That `max_tlen` is still passed unconditionally in the shared `input_to_fragments(...)` call inside `build_sub_fragments_h5`. Its removal is a known-fatal SE regression that has already been made and retracted once. **Confirmed**: `max_tlen=MAX_FRAG_LENGTH` is still passed unconditionally.
6. That no path resolution moved into the worker, which runs under `os.chdir` into a temp dir. **Confirmed** by inspection of the rebind block and worker body.
